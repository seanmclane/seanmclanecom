...
export async function getClimbsData() {
  'use server'
  const ratingCodes = {
  ...
  async function getData() {
    //don't revalidate this on tag revalidation so this function returns faster
  ...
  const recentClimbs = climbsJSON.slice(0,6)

  const hardestSport = climbsJSON.filter(c => 
    c.Rating_Code in ratingCodes.rock
    && c.Lead_Style
    && ['Onsight', 'Redpoint', 'Flash'].includes(c.Lead_Style)
    && c.Route_Type.includes('Sport')
  ).sort((a,b) => b.Rating_Code - a.Rating_Code)[0]

  const hardestTrad = climbsJSON.filter(c => 
    c.Rating_Code in ratingCodes.rock
    && c.Lead_Style
    && ['Onsight', 'Redpoint', 'Flash'].includes(c.Lead_Style)
    && c.Route_Type.includes('Trad')
  ).sort((a,b) => b.Rating_Code - a.Rating_Code)[0]

  const hardestIce = climbsJSON.filter(c => 
    c.Rating_Code in ratingCodes.ice
    && c.Lead_Style
    && ['Onsight', 'Redpoint', 'Flash'].includes(c.Lead_Style)
  ).sort((a,b) => b.Rating_Code - a.Rating_Code)[0]

  // mixed rating codes apparently don't work right, so use the number after "M"
  const hardestMixed = climbsJSON.filter(c => 
    c.Rating_Code in ratingCodes.mixed
    && c.Lead_Style
    && ['Onsight', 'Redpoint', 'Flash'].includes(c.Lead_Style)

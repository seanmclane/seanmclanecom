...
async function getPost(params: {slug: string}): Promise<QueryResponseInitial<PostType>> {
  ...
  `*[_type == "post" && slug.current == $slug][0]{
    ...
    persona -> {
      fullName,
      profileImage {alt, "image": asset->url, asset, hotspot, crop},
      headline
    },
    publishedAt,
...
export default async function Post({params}: Params) {
  ...
  const mainImageURL = urlForImage(post.mainImage).height(600).width(1200).url()
  const personaImageURL = urlForImage(post.persona.profileImage).height(600).width(1200).url()
